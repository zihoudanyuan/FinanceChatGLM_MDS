...
from method.prompt_generation_v2 import (
...
def process_question(question_obj,knowledge=None):
    ...
    if not compute_index:
        prompt_ = glm_prompt.handler_q(q=question_obj['question'],knowledge=knowledge)
        # print(prompt_)
        inputs_t = {'text': prompt_, 'history': []}
        response_ = pipe(inputs_t)['response']
        question_obj["answer"] = str(response_)
...
if __name__ == '__main__':
    ...
    for idx, question_obj in enumerate(tqdm(questions)):
        ...
        if question_obj['id'] in test_data_knowledge:
            process_question(question_obj,knowledge=test_data_knowledge[question_obj['id']])
        else:
            process_question(question_obj,knowledge=None)
