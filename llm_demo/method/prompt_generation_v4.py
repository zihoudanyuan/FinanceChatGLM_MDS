from query_functions import dq
import pandas as pd
...
formula_dict = {'企业研发经费与利润比值': '企业研发经费与利润比值=研发费用/净利润',
             '企业研发经费与营业收入比值': '企业研发经费与营业收入比值=研发费用/营业收入',
             '研发人员占职工人数比例': '研发人员占职工人数比例=研发人员数/职工总数',
             '流动比率': '流动比率=流动资产/流动负债',
             '速动比率': '速动比率=(流动资产-存货)/流动负债',
             '企业硕士及以上人员占职工人数比例': '企业硕士及以上人员占职工人数比例=(硕士人数 + 博士及以上人数)/职工总数\n或者 企业硕士及以上人员占职工人数比例=硕士以上人数/职工总数',
             '企业研发经费占费用比例': '企业研发经费占费用比例=研发费用/(销售费用+财务费用+管理费用+研发费用',
             ...
             '净资产增长率': '净资产增长率=(净资产-上年净资产)/上年净资产',
             '三费比重': '三费比重=(销售费用+管理费用+财务费用)/营业收入',
             '投资收益占营业收入比率': '投资收益占营业收入比率=投资收益/营业收入',
             '销售费用增长率': '销售费用增长率=(销售费用-上年销售费用)/上年销售费用',
             '财务费用增长率': '财务费用增长率=(财务费用-上年财务费用)/上年财务费用',
             '管理费用增长率': '管理费用增长率=(管理费用-上年管理费用)/上年管理费用',
             '研发费用增长率': '研发费用增长率=(研发费用-上年研发费用)/上年研发费用',
             '总负债增长率': '总负债增长率=(总负债-上年总负债)/上年总负债',
             '流动负债增长率': '流动负债增长率=(流动负债-上年流动负债)/上年流动负债',
             '货币资金增长率': '货币资金增长率=(货币资金-上年货币资金)/上年货币资金',
             '固定资产增长率': '固定资产增长率=(固定资产-上年固定资产)/上年固定资产',
             '无形资产增长率': '无形资产增长率=(无形资产-上年无形资产)/上年无形资产',
             '总资产增长率': '总资产增长率=(资产总额-上年资产总额)/上年资产总额',
             '营业收入增长率': '营业收入增长率=(营业收入-上年营业收入)/上年营业收入',
             '营业利润增长率': '营业利润增长率=(营业利润-上年营业利润)/上年营业利润',
             '净利润增长率': '净利润增长率=(净利润-上年净利润)/上年净利润',
             '现金及现金等价物增长率': '现金及现金等价物增长率=(现金及现金等价物-上年现金及现金等价物)/上年现金及现金等价物'}
...
class GLMPrompt:
    ...
    def handler_q(self, q, knowledge=None, pipe=None):
                ...
                for keywords, (template, data_func) in keyword_mappings.items():
                    if any(keyword in q for keyword in keywords) and contains_year:
                        ...
                        if data is None:
                            ...
                            # print('here!!!!!!!!!!!!!!!!!!!!!!!!')
                            if '研发人员占职工' not in q :#and '硕士及以上人员占' not in q:
                                return template_manager.get_template(template).format(context=f"{stock_info}\n{data}", question=q)
                            # elif '硕士及以上人员占'  in q and '硕士人数' not in data and '硕士以上人数' not in data:
                            #     return template_manager.get_template(template).format(context=f"{stock_info}\n{data}", question=q)
                            else:
                                # rewrite question
#                                 calculator_rewrite_prompt=f"""参考下面的例子改写输入的句子。
# 例子1：
...
# 输出：
# """
                                # calculator_rewrite_prompt = template_manager.get_template("question_rewrite_v1").format(question=q)
                                if '研发人员占职工' in q:
                                    rewrite_prompt_template = 'question_rewrite_v2'
                                # elif '硕士及以上人员占' in q:
                                #     rewrite_prompt_template = 'question_rewrite_v2_master'
                                else:
                                    assert 0, "wrong data type for python run"
                                calculator_rewrite_prompt = template_manager.get_template(rewrite_prompt_template).format(question=q)
                                rewrite_inputs_t = {'text': calculator_rewrite_prompt, 'history': []}
                                rewrite_response_ = pipe(rewrite_inputs_t)['response']
                                try:
                                    rewrite_response_ = rewrite_response_.replace('“','').replace('”','').replace('"','')
                                except Exception as e:
                                    print(e)
                                    print(rewrite_response_)
                                    rewrite_response_ = q
                                # print('**********'*2)
                                # print(q)
                                # print(rewrite_response_)
                                # print('**********'*2)
                                # return [template_manager.get_template("prompt_calculator").format(context=f"{stock_info}\n{data}", question=rewrite_response_),template_manager.get_template(template).format(context=f"{stock_info}\n{data}", question=q)]
                                if '研发人员占职工' in q:
                                    formula = "研发人员占职工人数比例=研发人员数/职工总数"
                                # elif  '硕士及以上人员占' in q:
                                #     # formula = "企业硕士及以上人员占职工人数比例=(硕士人数 + 博士及以上人数)/职工总数\n或者 企业硕士及以上人员占职工人数比例=硕士以上人数/职工总数"
                                #     # formula = "企业硕士及以上人员占职工人数比例=硕士以上人数/职工总数"
                                #     formula = "企业硕士及以上人员占职工人数比例=硕士以上人数/职工人数"
                                else:
                                    assert 0, "wrong data type for python run"
                                return [template_manager.get_template("prompt_calculator_v2").format(context=f"{stock_info}\n{data}", formula=formula, question=rewrite_response_),template_manager.get_template(template).format(context=f"{stock_info}\n{data}", question=q)]


                # 补充知识
                if knowledge is not None:
                    sample_context = f"{stock_info}\n{knowledge}"
                else:
                    sample_context = f"{stock_info}"
                # print(template_manager.get_template("basic_input").format(context=sample_context, question=q))
                return template_manager.get_template("basic_input").format(context=sample_context, question=q)
                # return template_manager.get_template("basic_input").format(context=stock_info, question=q)
        else:
            # print(template_manager.get_template("open_input").format(question=q))
