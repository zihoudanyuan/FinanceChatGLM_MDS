...
class DataQuery:
    ...
    def _row_to_sentence(row):
        ...
        other_data = {k: v for k, v in row.items() if k not in ['报告年份', '股票代码', '股票简称']}
    ...
    def query_balance_static_data(self):
        dummy_df = pd.read_excel(self.balance_static_path)
        dummy_df.rename(columns={'报告年份':'年份','股票代码':'证券代码','股票简称':'证券简称'},inplace=True)
        return dummy_df
